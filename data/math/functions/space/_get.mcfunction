#math:space/_get

scoreboard players operation space_startx int = @s space_startx
scoreboard players operation space_starty int = @s space_starty
scoreboard players operation space_startz int = @s space_startz
scoreboard players operation space_lenx int = @s space_lenx
scoreboard players operation space_leny int = @s space_leny
scoreboard players operation space_lenz int = @s space_lenz
scoreboard players operation space_lenxy int = @s space_lenxy
scoreboard players operation space_stepx int = @s space_stepx
scoreboard players operation space_stepy int = @s space_stepy
scoreboard players operation space_stepz int = @s space_stepz